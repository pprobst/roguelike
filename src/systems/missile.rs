...
use crate::components::{
    Player, ActiveWeapon, BaseStats, Equipment, MissileAttack, MissileWeapon, Name, SufferDamage,
};
use crate::log::Log;
...
pub fn missile(ecs: &SubWorld, commands: &mut CommandBuffer, #[resource] log: &mut Log, #[resource] rng: &mut RandomNumberGenerator) {
    let white = color("BrightWhite", 1.0);
    let active_missile_wpn = <(&ActiveWeapon, &MissileWeapon, &Equipment, &Name)>::query();
    let player = <&Player>::query();
    
    <(Entity, &MissileAttack, &BaseStats, &Name)>::query().iter(ecs).for_each(|ent, missile, attacker_stats, name| {
        commands.remove_component::<MissileAttack>(ent);

        let attacker_hp = attacker_stats.health.hp;
        let victim_stats = missile.target.get_component::<BaseStats>().unwrap();
        let victim_hp = victim_stats.health.hp;

        if attacker_hp > 0 && victim_hp > 0 {
            active_missile_wpn.iter(ecs).for_each(|_, missile_wpn, equip, name_wpn| {
                if equip.user == ent && missile_wpn.ammo.ammo > 0 {
                    ...
                    let victim_name = missile.target.get_component::<Name>().unwrap();
                    log.add(
                        format!(
