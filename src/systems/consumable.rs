...
use crate::components::{
    BaseStats, Consumable, ConsumeItem, Inventory, InventoryCapacity, Name, Player,
};
use crate::log::Log;
...
pub fn consumable(ecs: &SubWorld, commands: &mut CommandBuffer, #[resource] log: &mut Log) {
    ...
    <(Entity, &ConsumeItem)>::query()
        .iter(ecs)
        .for_each(|(ent, consume)| {
            let consume_item = ecs.entry_ref(consume.item).unwrap();
            if let Ok(item) = consume_item.get_component::<Consumable>() {
                let mut target_stats = ecs
                    .entry_ref(consume.target)
                    .unwrap()
                    .get_component::<BaseStats>()
                    .unwrap();
