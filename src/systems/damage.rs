...
pub fn damage(ecs: &SubWorld, commands: &mut CommandBuffer, #[resource] map: &mut Map) {
    <(Entity, &mut SufferDamage, &mut BaseStats, &Position)>::query().iter(ecs).for_each(|(ent, damage, victim_stats, pos)| {
        if !victim_stats.god {
